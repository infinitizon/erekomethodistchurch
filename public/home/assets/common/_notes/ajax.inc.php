...
if(isset($_POST['getPage'])){
    ...
    if(array_key_exists($link, $tableNm)){
        $sqlSearchQuery = "SELECT * FROM ".$tableNm[$link];
        $sqlGetTitles = $sqlSearchQuery." LIMIT 0";        
        $sqlSearchCount = "SELECT COUNT(*) count FROM ".$tableNm[$link];
        $rowsperpage = 10;
        ...
        $news .= "<div style=\"margin:5px;text-align:right;\">{$cr8Input}<a href=\"{$tableNm[$link]}\" class=\"goodBtn create\">Create New</a></div>";
        ...
        $output .= "</table>";  
    }elseif(in_array($link, array('/','/about-us','/about-us/our-vision','/youth-church','/youth-church','/youth-church/youth-of-excellence'
                            ,'/about-us/our-mission','/about-us/our-history/ereko-methodist-church','/youth-church/articles'
                            ,'/about-us/our-history/ereko-circuit','/about-us/our-history/methodism-in-nigeria'
                            ,'/church-organs','/church-organs/mens-fellowship','/church-organs/womens-fellowship','/church-organs/young-mens-fellowship'
                            ,'/church-organs/youth-fellowship','/church-organs/ladies-and-girls-fellowship','/church-organs/sunday-school-department-children'
                            ,'/church-organs/mens-fellowship/executive-officers','/church-organs/mens-fellowship/programmes-and-activities'))){
        ...
        $output .= "<input type=\"hidden\" name=\"dets\" value=\"\$det=array('t'=>'pages'"
                                                                    ...
                                                                    . ", 't_cond'=>array('type'=>'update'"
                                                                                    . ", 'col'=>array('page_label'=>'page_label'"
                                                                                                . ",'page_url'=>'page_url'"
        ...
        $output .= "<tr><td>Show in Menu</td><td>"
                        ."<select name=\"site_position\">"
                        . "<option value=\"1\"".(($content['site_position']==1)? " selected=\"selected\"":"").">Yes</option>"
                        . "<option value=\"0\"".(($content['site_position']==0)? " selected=\"selected\"":"").">No</option>"
                        ."</select>"
                        . "</td><tr>";
   ...
   //var_dump($_POST); exit;
    ...
    $targetFolder = array('lectionary'=>array('folder'=>'/assets/downloads/lectionary'
                                                ,'type'=>array('pdf'))
                        ,'hymn'=>array('folder'=>'/assets/downloads/hymn/'.  date('Y')
                                        ,'type'=>array('mp3'))
    ...
    if (in_array($fileParts['extension'],$fileTypes)) {
        ...
        echo '<blockquote style="color:#063">File uploaded successfully<br />File URL is '.WEB_ROOT.$targetFolder[$_POST['upload_tp']]['folder']."/".$_FILES['Filedata']['name']."</blockquote>";
    } else {
    ...
    if($table==='profiles'){
        ...
        $output .= "<input type=\"hidden\" name=\"dets\" "
                . "value=\"\$det=array('t'=>'{$table}'"
                                    . ", 't_u'=>array(".@$getCond2.")"
                                    . ", 't_cond'=>array('type'=>'".(!empty($editField)?'update':'insert')."'"
                                    . ", 'col'=>array('association'=>'association', 'president'=>'president', 'secretary'=>'secretary','tresurer'=>'tresurer','started'=>'started','yearAnniversary'=>'yearAnniversary')));\">";
        $output .= "<tr><td>&nbsp</h2></td><td>"
                . (!empty($editField)? "<a href=\"\" class=\"goodBtn update\" style=\"float:right;\">Update</a>":"<a href=\"\" class=\"goodBtn update\" style=\"float:right;\">Create Entry</a>")
                . "</td></tr>";        
        $output .= "<tr><td>Name</td><td><input type=\"text\" name=\"association\" value=\"".@$getEdit['association']."\" /></td><tr>";
        $output .= "<tr><td>Title</td><td><input type=\"text\" name=\"president\" value=\"".@$getEdit['president']."\" /></td><tr>";
        $output .= "<tr><td>Date</td><td><input type=\"text\" name=\"secretary\" value=\"".@$getEdit['secretary']."\" /></td><tr>";
        $output .= "<tr><td>Profile</td><td><input type=\"text\" name=\"tresurer\" value=\"".@$getEdit['tresurer']."\" /></td><tr>";
        $output .= "<tr><td>Image Url</td><td><input type=\"text\" name=\"started\" value=\"".@$getEdit['started']."\" /></td><tr>";
        $output .= "<tr><td>Image Url</td><td><input type=\"text\" name=\"yearAnniversary\" value=\"".@$getEdit['yearAnniversary']."\" /></td><tr>";
    }elseif($table==='news'){
        $output .= "<input type=\"hidden\" name=\"dets\" "
                . "value=\"\$det=array('t'=>'{$table}'"
                                    . ", 't_u'=>array(".@$getCond2.")"
        ...
        $output .= "<tr><td>Active</td><td>"
                    ."<select name=\"active\">"
                    . "<option value=\"1\"".((@$getEdit['active']==1)? " selected=\"selected\"":"").">Yes</option>"
                    . "<option value=\"0\"".((@$getEdit['active']==0)? " selected=\"selected\"":"").">No</option>"
                    ."</select>"
                . "</td><tr>";
    }    
    $output .= "</table>";        
